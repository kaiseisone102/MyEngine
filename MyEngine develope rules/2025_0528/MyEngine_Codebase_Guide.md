...
- 描画パス (全て RAII 化済み):
  ...
  - `main_pass.*` — **本体描画 (シーン)。** static/skinned/grass/bindless の opaque/transparent、water、debug line、particle を束ねる。 **【PART4 4a-1 完了 commit af3dd72 / 4a-2 完了 commit ed0d80e】**: VkRenderPass + VkFramebuffer を撤去し Vulkan 1.3 dynamic rendering (`vkCmdBeginRendering` + `VkPipelineRenderingCreateInfo`) に。 opaque は **3 attachment MRT** (HDR + GBuffer normal R10G10B10A2 octahedral + motion vector RG16F) + 深度、 非 opaque は 1 attachment HDR + 深度の **2 scope に分割** (Vulkan VUID-06195 整合)。 child pipeline (debug_line / particle / water) も同経路 (format ベース) で rebuild。 **HUD/ImGui は OverlayPass に分離した (`mainPass_` の execute からは出た)**。 depth layout は `depth_layouts::attachment/readOnly(*ctx_)` 経由 (separate / combined フォールバック)。
  ...
  - `depth_layouts.h` — **【PART4 4a-2 で新設 (header-only), commit ed0d80e】depth 画像の attachment / read-only layout を一元化する namespace。** `attachment(ctx) → VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL` / `readOnly(ctx) → VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_OPTIMAL` (Vulkan 1.2 separate depth/stencil layouts、 D32_SFLOAT に正確)。 `ctx.separateDepthStencilLayouts()` (vulkan_context 側 query 結果) が false のとき combined `DEPTH_STENCIL_*_OPTIMAL` に自動フォールバック。 main_pass と widget が両方ここを呼ぶことで layout 選択がズレない。
...
### shaders/
- `shaders/shared/types.h` — **C++/GLSL 共有の構造体定義。** PushConstants 各種・InstanceData・**DrawData (1K相当の per-draw SSBO 要素, 80B) / StaticDrawPushConstants (PART3b, 8B/VERTEX, DrawData SSBO アドレスのみ)** 等。C++ と GLSL の両方から include される単一の正。 **【PART4 4a-2, commit ed0d80e】FrameUBO に `mat4 prevViewProj` を追加 (合計 416B)** — opaque vertex shader が motion vector 計算用に前フレーム VP を引く。 host (`VulkanRenderer::buildCompleteFrameUBO`) は前フレーム末尾の `proj * view` を `prevViewProj_` にスナップし次フレームに渡す。 **struct size 変更なので Work_Protocol §3-1a に従い clean rebuild 必須** (4a-2 で incremental build を続け mode select → 本編で TDR を起こした実例あり)。
...
## 3.6. PART4: Hi-Z occlusion 受け皿 (4-前 / 4a 完了, 4b 以降残)

PART4 §6 の「4-前-0 〜 4-前-5 + 4a-1 + 4a-2 = 完了」。 設計詳細は side/MyEngine_HiZ_PART4_Design.md (作業正本)。
...
- **4a-2 (depth-normal-motion MRT + OverlayPass 分離, ed0d80e)**: opaque を MRT 3 attachment (HDR + GBuffer normal R10G10B10A2 octahedral + motion vector RG16F) + 深度に拡張、 swapchain depth は SAMPLED_BIT 付き。 HUD/ImGui を OverlayPass という新 dynamic-rendering pass に分離 (HudPipeline / ImGui pipeline は `depthAttachmentFormat = VK_FORMAT_UNDEFINED` で rebuild、 OverlayPass の BeginRendering は color-only)。 `gbuffer_debug_widget` (右上ドック ImGui::Image viewer)、 `depth_layouts.h` (separate / combined フォールバック)、 `shared/gbuffer.glsl` (octahedral encode + motion vector) 新設。 FrameUBO に `mat4 prevViewProj`。 capability 追加: `separateDepthStencilLayouts`。 **教訓 (Work_Protocol §3-1a)**: FrameUBO struct size 変更で clean rebuild 必須だったのに incremental build を続け、 一部 .obj が古い 352B 想定で残り mode select → 本編で UBO ズレ → rasterizer hang (TDR) → 画面凍結を起こした。 共有ヘッダ struct size 変更は **必ず clean rebuild**。

**次 = PART4 4b (HZB SPD)** → 4c (two-pass occlusion 本体) → 4d (能力ゲート集約 + DGC / Shader Object / Descriptor Buffer / Timeline semaphore / Async compute 受け皿 + RenderTarget 抽象 + 一時ログ掃除)。
