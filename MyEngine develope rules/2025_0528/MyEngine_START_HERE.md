...
- **直近の完了**:
  - **Phase 2G-1 + 2G-2a 完了 (2026-05-30, HEAD 1b57f8e・user runtime 検証済)**: skinned を §1.5-B 「すべて GPU-driven」へ寄せる前半。
    ...
    - **残**: 2G-2b (skinned を CullingPass CullSet + vkCmdDrawIndexedIndirect = CPU draw ループ撤去で §1.5-B 完全準拠) / 2G-3 (motion vector double-buffer + LDS bone cache + async overlap)。 詳細は Roadmap §4 Phase 2G / INDEX §1。
