...
- **すべての描画 (static prop / terrain / 将来は skinned も) を最終的に compute カリング + indirect draw に乗せる。** CPU draw ループが残るものは無くす (スケールしないため)。これが「大規模エンジン」の核。
  - **射程の明示 (2026-05-30 監査で確定)**: prop = ✅ 完了 (Phase 2B)。 **skinned = Phase 2G** (現状 CPU draw ループ + vertex-shader skinning 3〜4 回再実行 = §1.5-B 違反・★次推奨)。 **grass = Phase 2H** (現状 CPU per-blade frustum cull = 違反)。 terrain = Phase 2F (別 bucket)。 残る CPU ループ (water / bindless test / reflection static / transparent) は後段。 「将来は skinned も」の「将来」= Phase 2G として番号確定。
