...
> - **教訓 (Work_Protocol §3-1a 違反した)**: 共有ヘッダ struct size 変更 (FrameUBO に prevViewProj 追加 = 352B → 416B) で **clean rebuild 必須だったのに incremental build を続けた**結果、 一部 .obj が古いサイズで残り、 mode select → 本編遷移時に UBO のズレで rasterizer hang (TDR) → 画面凍結。 §3-1a の徹底必須。
>
...
>
> **次にやること = PART4 4a-2 完了 (受け皿全部立った)。次は 4b HZB SPD 本体着手**:
> - **PART4 4b** (HiZPass 新設 = AMD FidelityFX SPD で min+max ペア HZB を 1 dispatch 生成、`hiz_spd.comp` 新規)。 main_pass 直後の深度 RT を入力にし、 R32G32_SFLOAT (min/max) per-frame mip chain を作る。 4c の two-pass occlusion 本体に直結する。設計詳細: side/MyEngine_HiZ_PART4_Design.md §6 「4b」。
> - **PART4 4c** (two-pass occlusion 本体 + AABB 遮蔽 + cull.comp 拡張) — 4b の HZB を読んで遮蔽判定する核。
...
- **次の推奨の一手 = PART4 4a-2 完了 (commit ed0d80e)。次は 4b HZB SPD 本体 (side/MyEngine_HiZ_PART4_Design.md §6 「4b」)。** 4b は SPD で 1 dispatch min+max ペア HZB を生成する compute pass の新設。 詳細は上の「▶ 前回セッションからの引き継ぎ」の「次にやること」を参照 (4b / 4c / 4d / 並行可候補)。設計判断は記憶でなく資料を引いてから (§0-8)。着手時にまず最新動向を web 確認。
