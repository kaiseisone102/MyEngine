...
> ### ▶ 前回セッションからの引き継ぎ (新規チャットはまずここを読む / 2026-05-28 更新)
>
> **【PART4 4b 完了 = HiZPass 新設 (SPD-style single-dispatch min+max RG32F pyramid) / 2026-05-28, commit pending】**
...
> - **capability 追加 (Design §1.5-C 「実測してから書く」)**: `subgroupOps` (basic + arithmetic + quad in COMPUTE)・ `subgroupSize`・ `shaderStorageImageArrayDynamicIndexing` (HiZPass の per-mip storage image array を loop-uniform 動的 index で引くため必須)。 P620 実測: `subgroupOps=1 subgroupSize=32`。 wave-ops 高速版 .comp は受け皿として用意 (`useWavePath_`)、 現状は LDS-only 単一 spv を両経路で使う (中身を後から差し替えられる構造・§0「受け皿を先に最新で」)。
> - **pass_chain 配線**: `mainPass_.execute()` 直後・ `overlayPass_.execute()` の前 (depth が DEPTH_READ_ONLY_OPTIMAL に遷移済みのタイミングで sample、 mid-pass barrier 不要のクリーン経路)。 ExecuteInfo は `cmd + frameIndex` のみ。 `onSwapchainResized` で pyramid を depth 解像度に追従させて rebuild (pipeline / set layout は extent-independent で keep)。
> - **検証**: `Vulkan ready` + 全 capability 検出 (`subgroupOps=1 subgroupSize=32 dynamicRendering=1 separateDepthStencilLayouts=1`)・ HiZ 関連 VUID/leak/device-lost **ゼロ**・ pre-existing validation warning は 4a-2 由来の outNormal/outMotion 1-attachment subpass 警告のみ。 描画見た目 (HZB viewer の表示・min/max が mip ごとに保守的に縮約) はユーザー目視待ち。
...
> - **純 GPU-driven 化の仕上げ (任意)**: CullingPass の CPU オラクル + readback (PART2 検証用) を撤去し、CPU が可視数を知らない形にする。 4c 着手の直前にやるのが素直。
...
- **次の推奨の一手 = PART4 4b 完了 (commit pending)。次は 4c (two-pass occlusion 本体 = side/MyEngine_HiZ_PART4_Design.md §6 「4c」)。** 4c は CullingPass を `executePass1` / `executePass2` に分け、 4b の HZB を読んで AABB occlusion 判定する Hi-Z の核。 cull.comp に HZB sampler + AABB 画面投影を追加、 `static_cull_build.h` で half-extent 充填、 pass_chain が「パス1 cull → 描画 → 4b → パス2 cull → 描画」をオーケストレート。 詳細は上の「▶ 前回セッションからの引き継ぎ」の「次にやること」参照 (4c / 4d / 並行可候補)。 設計判断は記憶でなく資料を引いてから (§0-8)。 着手時にまず最新動向を web 確認。
