...
- 起きた事例② (2026-05-28, PART4 4a-2 着手時): types.h の `FrameUBO` に `mat4 prevViewProj` を追加 = 352B → 416B (motion vector 受け皿)。 4a-2 の長い実装期間中ずっとインクリメンタルビルドを続け、 完成後の動作確認で **「タイトル/メニュー画面は描画されるが、 mode select → 本編 startGame の遷移で画面が固まる (TDR / GPU hang)」** という症状を起こした。 mode select 時はオブジェクトが少なく UBO のズレが見えない範囲で済んでいたが、 本編で多数の opaque static / skinned が描かれた瞬間に古い 352B 想定の .obj が参照する `prevViewProj` 領域 (= UBO buffer 末尾 64B = 古い .obj では未定義領域) を GPU が読み、 vertex shader の `gl_Position` が NaN / 巨大値になって rasterizer hang。 **症状の出方**: validation error 一切出ず、 driver TDR で画面凍結 (run.log にも痕跡なし)、 「mode select → 本編」遷移という特定条件でのみ起きる = デバッグ難度最高クラス。
