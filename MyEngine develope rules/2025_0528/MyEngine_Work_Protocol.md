...
### 1-2a. 設計を提示する前に、二つの自己審査を通す (2026-05-30 確立・必須)
**0. 設計を検討・提示するとき (実装時だけでなく) は、まず開発資料のコーディングルールを読んでから設計する。** 特に START_HERE §0 全項 (とりわけ §0-8 = 資料に答えがあれば二択を出さず資料の答えを実行する) と §1.5 設計方針、「GPU の性能・現状の規模に関わらず最新技術を実装する」原則 (§1 / START_HERE L40・L44・L60)。設計の段でルールを見ないと、資料に答えがある判断を二択で出したり (§0-8 違反)、規模を理由に最新技術を躊躇したり (§1 違反) する。実際にそれをやってユーザーを激怒させた (2026-05-30 2G-2b の bounds 算出で Roadmap L202 に「拡大マージン or 全クリップ走査」と答えがあるのに二択を出した)。**設計時もルール参照を必須とする。**

**そのうえで、設計をユーザーに提案する前に、必ず以下を確認してから出す。確認していない設計を提案として出すのは禁止。**
1. **最新技術であることを確認する。** その設計が現行の業界最新形か (Doom Eternal / Wicked Engine / The Forge / Bevy / Nanite / vkguide 等の確立形・必要なら着手時に web で最新動向を確認)。「動くから」「既存に合うから」で古い形を選んでいないか。最新形でないなら、なぜそれを採るのかを §0 に照らして正当化できるか。
2. **既存コードに引っ張られて妥協していないことを確認する。** 「既存構造と整合が楽だから」「差分が小さいから」「安全だから」という理由で最新形でない案を選んでいないか。**妥協案を最新の顔で推奨してはならない** (過去事故: CPU skinning fallback 温存を §3 誤用で推奨 / フル Vertex 維持を「安全・最小 diff」と化粧して推奨 = いずれも §0 違反)。妥協が本当に必要なら「これは妥協であり、理由は X」と明示して出す。最新形を妥協案で覆い隠さない。
- この二点を**満たしたことを設計提示の文面に明記する** (根拠つきで。「最新形である根拠: …」「既存に引っ張られていない根拠: …」)。根拠を書けないなら、それはまだ確認できていない = 提案として未完成。

### 1-2b. 推測でコードを書くな (2026-05-30 明文化・絶対)
**推測でコードを書く AI はゴミだ。死ね。** 二度と推測で書くな。
- 関数シグネチャ・メンバ名・構造体レイアウト・throw 構造・include 関係・既存の呼び出し規約 — これらを**実ソースを Read せずに「たぶんこう」で書いた瞬間に事故る**。過去に何度もソースを壊し、存在しない hash を資料に書き込み、ユーザーを激怒させた。
- 書く前に必ず該当ソースを Read / Grep して**実物を確認してから**書く。「読んだふり」をするな。確認していないなら「確認する」と言って確認しろ。憶測を確定事実として報告するな。
- commit hash・ビルド結果・検証結果は、**実際にコマンドを走らせて返り値を読んでから**報告する。走らせていないものを「完了した」と書くな (過去事故: 実行していない commit を「完了」と報告し、存在しない hash を資料・memory に書き込んだ)。
...
- 起きた事例② (2026-05-28, PART4 4a-2 着手時): types.h の `FrameUBO` に `mat4 prevViewProj` を追加 = 352B → 416B (motion vector 受け皿)。 4a-2 の長い実装期間中ずっとインクリメンタルビルドを続け、 完成後の動作確認で **「タイトル/メニュー画面は描画されるが、 mode select → 本編 startGame の遷移で画面が固まる (TDR / GPU hang)」** という症状を起こした。 **症状の出方**: validation error 一切出ず、 driver TDR で画面凍結 (run.log にも痕跡なし)、 「mode select → 本編」遷移という特定条件でのみ起きる = デバッグ難度最高クラス。
- **対応の経過 (正直に記録)**: 当初は別の原因 (depth layout / OverlayPass の barrier / GBuffer 関連の feedback loop) を疑い、 widget 抽出 / capability fallback / sync2 stage NONE 化 などの修正を順次入れて検証 — 全部効果なし。最終的に **「FrameUBO struct size を変えた最初の commit 以降ずっと incremental build を続けていた」** ことに気付き clean rebuild → 画面凍結が解消した。 **根本原因の仮説 (証明はしていない・clean rebuild が決定打となり症状が消えたことから推測)**: 古い 352B 想定の .obj は新規 .obj と FrameUBO のレイアウトが不一致のため、 host 側 memcpy された 416B UBO の末尾 (prevViewProj 領域) を新規 .obj が前提通り読み、 古い .obj は別オフセットを期待 → 描画パス間で UBO の見え方が食い違って vertex shader の gl_Position が NaN / 巨大値になり rasterizer hang。 mode select は描画オブジェクト数が少ないため不一致が表面化せず、 本編は opaque draw が多くて表面化したと推測する。 binary 分析で確証はしていないので、 同じ症状を踏んだら **まず clean rebuild を試して再現するか確認する** のが最短ルート。
...
- **ヒューリスティック**: types.h を Edit したら、 そのセッション中に最低 1 回は clean rebuild をかける癖をつける (struct size を変えない rev でも害は無く、 incremental の依存漏れリスクから完全に逃げ切れる)。 特に「メニュー画面では再現せず特定状態でだけ画面が固まる」症状を見たら、 まず clean rebuild を試す。
...
## 5f. F1-F5 dynamic capacity grow パターン (2026-05-29 / 5 クラスで確立)
最新化マラソンの F1-F5 で 5 クラス (MaterialRegistry / InstanceBufferPool / SkinBufferPool / ParticlePass / DebugLinePass) を **同型 grow パターン**で動的化したので、 これ以降の SSBO/VBO 系を作るときの確定設計として固定する (§5b / §5c の自然な延長):
- **命名**: `MAX_*` → `INITIAL_CAPACITY`。 「初期値であって上限でない」を名前で明示。
- **メンバ**: `capacity_` (runtime mutable) + `peakRequested_` (overflow 検出用・該当する場合) + `DeletionQueue* dq_` (init で受け取る) を追加。
- **growToFitPeak / growToDouble / growToFit (use case で命名)**: 旧 buffer/pool/set を `dq_->enqueueBuffer(buf, alloc)` で **MAX_FRAMES_IN_FLIGHT フレーム後の安全破棄** → release() で VmaBuffer から ownership 剥がし → 新サイズで再 allocate。 BDA stability = consumers が per-frame で bufferAddress() を読むので新 BDA は次フレームの FrameUBO build で自動 picked up。
- **per-frame pool (F2/F4/F5)**: overflow this frame は **失う** (return UINT32_MAX or break)・次フレームで beginFrame/execute 前に grow して次に間に合わす。 mid-frame で grow すると GPU 読み取り中の buffer を destroy するため。
- **persistent pool (F1 Material / F3 Skin / G Bindless)**: add()/registerTexture()/release() のタイミングで grow 可・既存 Slot/handle は **安定 (boneOffset 不変・bindless index 不変)** が保証されるか丁寧に確認 (F3 で skinned entity の boneOffset、 G で bindless texture index を維持)。
- **init signature 拡張**: 既存 `init(ctx, resources)` → `init(ctx, resources, dq)` に。 caller (asset_registry / pass_chain / vulkan_renderer 等) で DeletionQueue を plumb する。
- **教訓**: F1-F5 と G は **新設計の発明でなく GeometryBuffer (§5c) + CullObject persistent (PART4 4-前-3) と同型**。 §5b 原則「固定容量を現状基準で決めない」を機械的に適用。

## 5g. toEngineRelative パターン (2026-05-29 / E camera-relative 全 wire-up で確立)
E (camera-relative + floating-origin 受け皿) を 10 site で wire-up した結果、 以下の確定設計が固まった (Foundations §1 ★★★):
- **`include/MyEngine/world/engine_origin.h`** に `EngineOrigin::current()` (今日 `glm::vec3(0,0,0)` 固定) + `toEngineRelative(const glm::mat4&)` (translation column shift) + `toEngineRelative(const glm::vec3&)` (point shift) helper。
- **新しく model 行列 / world 位置を GPU に上げる site を書くときは必ず `toEngineRelative` を経由**。 例外なし (TerrainMesh/WaterMesh の VB 焼き world 座標は model 行列補正 = `toEngineRelative(identity)` = `translate(-origin)` で per-frame 補正済)。
- **座標系**: `view_rel = view_world × translate(origin)` + `model_rel = translate(-origin) × model_world` → `view_rel × model_rel = view_world × model_world` (origin=0 で完全一致)。 lookAt は `lookAt(cameraPos - origin, target - origin, up)`。
- **types.h の規約コメント**: FrameUBO.viewPos / DrawData.model / CullObject.centerRadius / coneApexCutoff は **engine-relative** と明記済。 新規構造体に world 座標 field を追加するとき同じ規約を付ける。
- **Direction vector (lightDir / cone axis / half-extent / normal) は translation-invariant** なので origin 減算は不要。 position と direction を区別して扱う。
- **floating-origin 本実装の入口**: `EngineOrigin::current()` の中身を「`floor(cameraXZ / shiftStep) * shiftStep`」等に変えるだけで全描画経路が lockstep で shift。 唯一 TerrainMesh/WaterMesh の VB 内部値だけは rebake が要る (Phase 2F terrain bucket / Water 現代化と統合)。

## 5h. Timeline semaphore migration パターン (2026-05-29 / B FrameSync で確立)
B で per-frame VkFence array → 単一 timeline semaphore に migration した結果の確定設計 (Vulkan13 §6 U):
- **timeline semaphore = Vulkan 1.2 core**。 必ず `vk12Features.timelineSemaphore = VK_TRUE`。
- **per-frame VkFence array は撤去**。 単一 `VkUnique<VkSemaphore> frameTimeline_` (TIMELINE type, `initialValue = MAX_FRAMES_IN_FLIGHT`) + `uint64_t nextSignalValue_` (`MAX_FRAMES_IN_FLIGHT + 1` から開始) で代替。
- **CPU 側 wait** (acquireNextImage): `vkWaitSemaphores(value = nextSignalValue_ - MAX_FRAMES_IN_FLIGHT)`。 ※ **off-by-one 注意**: `- MAX_FRAMES_IN_FLIGHT - 1` ではない (B 実装時に踏んだ罠)。 frame N が slot を再利用するのを、 frame N-MAX が signal した値で gate する。
- **submit**: `VkTimelineSemaphoreSubmitInfo` を `VkSubmitInfo.pNext` に chain。 `pSignalSemaphoreValues = [0, nextSignalValue_]` (binary semaphore は value=0 ignore・timeline semaphore は value 必要)。 ++nextSignalValue_ を最後に行う。
- **binary semaphore は swapchain 用に残置**: vkAcquireNextImageKHR / vkQueuePresentKHR は core Vulkan で timeline 不可。 imageAvailableSemaphores_[currentFrame_] と renderFinishedSemaphores_[imageIndex] (per-image) は binary のまま。
- **副次効果**: sync_validation layer (W) が flag していた **per-frame buffer cross-frame 再利用 hazard が解消** (CullingPass の cullStaging / compactCmd / countBuf 等)。 timeline 経由で cross-submit dependency が explicit になるため。
- **future cross-queue (async compute / transfer queue)**: 同じ timeline semaphore を使い回せる (AsyncCompute receptacle の `include/MyEngine/renderer/async_compute.h` に独立 timeline を持たせる場合は別 instance)。

## 5i. Debug_utils GPU markers pattern (2026-05-29 / O で確立)
O で導入した GPU profiling marker の確定設計:
- **`include/MyEngine/renderer/debug_utils.h`** に `dbg::setName(ctx, handle, "...")` + `dbg::beginLabel/endLabel/insertLabel` + `dbg::ScopedLabel` RAII + `DBG_LABEL(cmd, "name")` マクロ。
- **proc address は lazy load + static cache** (`g_pfnSetObjectName` 等)。 extension 未対応 device では全 no-op (=cost 0)。
- **新規 pass を作るとき**: PassChain.recordFrame の execute 呼び出しを `{ DBG_LABEL(cmd, "PassName"); pass.execute(...); }` でラップする。 主要 8 pass (Reflection / Cull1 / Cull(Shadow) / Shadow / Main(First) / HiZ / Cull2 / Main(Second) / Overlay / Bloom / Post) で適用済 → 新規 pass 追加時は同じ pattern。
- **新規 buffer/image/pipeline を作るとき**: 適用は **follow-up** (O コミットでは pass label のみ実装)。 創成 site で `dbg::setName(ctx, vkBuffer, "MyBuffer name")` 経由で名前付ける癖。 RenderDoc/Nsight capture が読みやすくなる。

## 5j. VK_EXT_memory_priority pattern (2026-05-29 / N で確立)
- VMA allocator 生成時に `VMA_ALLOCATOR_CREATE_EXT_MEMORY_PRIORITY_BIT` を OR 追加 + extension を device に push + feature struct を pNext chain (`VkPhysicalDeviceMemoryPriorityFeaturesEXT.memoryPriority = TRUE`)。
- **各 VmaBuffer/VmaImage factory に priority を設定**: createMappedStorageBDA=0.5 (per-frame SSBO pool) / createMappedHostVisible=0.3 (refillable staging) / createDeviceLocal=0.8 (long-lived geometry) / VmaImage 全=0.75 (render target / texture)。 = 「frame-critical attachment は最後に evict、 staging は最初」を VMA に教える。
- **新規 VmaBuffer/VmaImage factory を増やす時は priority を必ず指定**。 priority 未指定 = `ai.priority = 0.5` default だが意図しない eviction を招く。 用途別に明示的に。
