...
#### Phase 2G — skinned の GPU-driven 統一 (compute skinning + indirect) 【新規・未着手】
...
**現状 (2026-05-30 実ソース監査で確定・§1.5-B 違反)**: skinned 描画は全経路 CPU draw ループ:
...
**PART 分割 (§1-2 細かく commit・怖い変更は変更前後提示)**:
- **2G-1 受け皿**: types.h に SkinningPushConstants + per-instance skinned vertex メガバッファ (deinterleaved pos ping-pong / normal / static attr 共有) を SkinnedVertexPool として再設計実装 + skinning.comp (LDS bone cache・spec const) + compute pipeline。dispatch 配線だけで draw は旧経路のまま = **描画不変の no-op 受け皿**。検証: ビルド・validation 0・描画不変。
- **2G-2 activate**: passthrough vert 3 本 (main/shadow/reflection) + conservative animation bounds 算出 + skinned を CullingPass の新 CullSet に乗せ indirect 化 + 旧 drawSkinnedList / 旧 vertex-skinning 撤去。検証: knight アニメ/影/反射 正常 (目視必須)・validation 0。
- **2G-3 仕上げ**: motion vector double-buffer の main pass 接続 (TAA/motion blur 受け皿完成) + async compute overlap の M wire (実 submit は後段)。検証: 目視 + validation 0。
...
8. **Phase 2G (skinned GPU-driven 統一)** ← ★**2026-05-30 監査で最優先に格上げ**。 skinned 描画が全経路 CPU draw ループ = §1.5-B 違反 (看板「最新 GPU-driven」/ 実体「skinned だけ CPU 旧経路」)。 compute skinning (packed VB) → prop と同じ CullingPass / indirect に統合 + 旧 vertex-shader skinning 撤去。 §0 最優先原則 (古い技術の投棄・妥協なし) の直接適用。 詳細は §4 Phase 2G。
