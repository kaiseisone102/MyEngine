...
#### Phase 2G — skinned の GPU-driven 統一 (compute skinning + indirect) 【2G-1 + 2G-2a 完了 (2026-05-30, HEAD 1b57f8e)・残 2G-2b/2G-3】
...
- **残 2G-2b**: skinned を CullingPass の新 CullSet に乗せ `vkCmdDrawIndexedIndirect` 化 = CPU draw ループ (drawSkinnedPrepared) 撤去 + conservative animation bounds。 これで §1.5-B 完全準拠。
...
8. **Phase 2G (skinned GPU-driven 統一)** ← **2G-1 + 2G-2a 完了 (2026-05-30, HEAD 1b57f8e・runtime 検証済)・残 2G-2b ★次推奨**。 skinned 描画が全経路 CPU draw ループ = §1.5-B 違反だったのを是正中。 ✅ 2G-1 = batched compute skinning pre-pass (skin once の土台)。 ✅ 2G-2a = passthrough vert で skinned stream を読み旧 vertex-shader skinning 撤去 = **skin once 達成 (3〜4x→1x)** + SkinnedDrawData SSBO で indirect-ready。 ⬜ 2G-2b = skinned を CullingPass CullSet + vkCmdDrawIndexedIndirect (CPU draw ループ撤去 = §1.5-B 完全準拠) ⬜ 2G-3 = motion vector double-buffer / LDS bone cache / async overlap。 詳細は §4 Phase 2G。
