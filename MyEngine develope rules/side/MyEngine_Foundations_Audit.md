# MyEngine オープンワールド土台監査 — 先回りすべき受け皿 / 先送り負債 (rev.4)
...
## 5. 【中・PART4 に今すぐ織り込める】描画アーキ — depth-normal prepass の共有
...
### 受け皿 (今すぐ・PART4 4a に追記)
**Hi-Z (PART4) が深度を保存する最初の消費者**。だから 4a で深度を STORE/SAMPLED 化するとき、**法線も出す軽量 GBuffer (depth + normal) として設計**しておけば、SSAO/SSGI/SSR/Hi-Z が **1 枚の prepass を共有**できる。各 SS フェーズが個別に深度/法線パスを後付けする手戻りを防ぐ。
- 注意: これは forward を deferred に変える話ではない。**forward+ を保ちつつ、SS 効果と Hi-Z が共有する depth(+normal) prepass を持つ**ハイブリッド (Doom 2016 / UE4 forward+ と同系)。MSAA/透明の forward の長所は維持。

### 確認用 (PART4 設計書で確認済み)
main_pass の深度は今 `storeOp=DONT_CARE` で破棄 (PART4 §1①)。法線を G-buffer 的に出しているかは未確認 (依存マップ §3 1J の「要確認: 法線を G-buffer 的に出しているか」と同じ宿題)。

### 危険度: ★★ (中)。ただし **PART4 4a に織り込めば追加コストはほぼゼロ** (深度を保存するついでに法線も)。先送りすると各 SS フェーズで retrofit。
...
5. ★★ depth-normal prepass 共有 (**PART4 4a に今すぐ織り込める**)
...
3. **PART4 設計書 (rev.2) の 4a に「depth-normal prepass 共有 (項目 5)」を追記** ── これは今すぐ効く小さな一手で、深度保存のついでに法線も出すだけ。
