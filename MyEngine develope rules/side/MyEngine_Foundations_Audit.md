...
- 🟡 **skinned (CPU draw ループ → 2G で解消中・2026-05-30, HEAD 1b57f8e)**: 旧状態 = opaque/transparent/shadow/reflection 全経路 CPU draw ループ + vertex-shader skinning 3〜4 回再実行。 → ✅ **2G-1** で batched compute skinning pre-pass (skin once の土台) + ✅ **2G-2a** で passthrough vert (skinned stream を読み旧 vertex-shader skinning 全撤去) + SkinnedDrawData SSBO (indirect-ready) = **頂点 skinning の 3〜4x 再実行を 1x に解消済み・user runtime 検証済**。 ⬜ 残 **2G-2b** = skinned を CullingPass CullSet + vkCmdDrawIndexedIndirect (CPU draw ループ drawSkinnedPrepared を撤去 = §1.5-B 完全準拠) で 🔴→🟢 完了。 詳細 = Roadmap §4 Phase 2G / INDEX §1。
