...
| - | (T) Dynamic rendering (main_pass + Overlay) | 4a-1 / 4a-2 | 中 (main_pass の VkRenderPass 撤去 + 子 pipeline format ベース化 + OverlayPass 分離) | 大 (Vulkan 1.3 modern triad 完成・MRT 拡張前提) | ✅ 完了 (af3dd72 + ed0d80e、 他 pass の段階移行 + RenderTarget 抽象は 4d) |
| 3 | (Y) Pipeline cache 永続化 | PART4 と並列・いつでも | 低 (数十行) | 中 (起動時間短縮 / hitching 解消) | 🟢 未着手 |
