...
### 4-前-0 — Reverse-Z depth 全面切替 (Hi-Z 着手前提・全 pass 波及)
...
### 4-前-1 — builder の block sort + main_pass の区間検出撤去 + BlockRange 配列導入 (シェーダ無改修)
...
### 4-前-2 — CullObject の meshlet-ready 拡張 (§3.2-G) + builder で cone 充填 + cull.comp に cone test
...
### 4-前-3 — Persistent CullObject buffer (§3.2-K) + CullingPass / DrawDataPool に grow 経路 + visBuf 確保
...
### 4-前-4 — Scan compaction (§3.2-I) + `compactCmd1/2Buf_` + `countBuf1/2_` + main を IndirectCount 化 + DGC 受け皿
...
### 4-前-5 — Shadow_pass の GPU-driven 化 (§3.2-J)
...
### 4b — HiZPass 新設 = SPD (Single Pass Downsampler・§3.3-M) で min+max ペア (§3.3-N) 生成
**rev.4 の "BloomPass 雛形" を撤回し SPD で実装**。
- `renderer/hiz_pass.*` 新設。1 dispatch で全 MIP 生成。
- 新 compute shader `hiz_spd.comp` 作成: AMD FidelityFX SPD GLSL 版ベース + min/max user-defined 2x2 reduction。
- HiZPyramid フォーマット = `VK_FORMAT_R32G32_SFLOAT` (min+max ペア)。per-frame 2 枚。
- atomic counter buffer (per-frame の reset を pass_chain で行う)。
- subgroup ops 対応 query で 2 経路 (subgroup ops + LDS / LDS-only)。
- pass_chain で MainPass 直後 (パス1 描画後の HZB 生成タイミング・後の 4c で two-pass フロー)。
- **cull は無改修。** 検証: pyramid の各 mip を可視化し、min/max が正しく出ているか確認。1 dispatch で完了することを RenderDoc で確認。validation/leak ゼロ。
- **commit 単位**: 1 commit。
