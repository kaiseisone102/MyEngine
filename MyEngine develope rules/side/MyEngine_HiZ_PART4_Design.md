...
### 4b — HiZPass 新設 = SPD (Single Pass Downsampler・§3.3-M) で min+max ペア (§3.3-N) 生成 ✅ **完了 2026-05-28 commit pending**
...
ソース再点検 (2026-05-28、 commit pending 直前) で見つかった改善余地。 correctness bugs は全て修正済み (Bug 1 / 2 / 3 + Obs A / E + kMaxFramesInFlight 一元化)。 以下は **動作は正しいが「より正しい」形にできる**項目。 ユーザー判断で 4b スコープでは見送り。
