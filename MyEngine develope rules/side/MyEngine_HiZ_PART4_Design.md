...
- **教訓 (Work_Protocol §3-1a)**: FrameUBO struct size 変更で clean rebuild を怠ると、 mode select → 本編 startGame 遷移時のみ GPU hang (画面凍結) を起こす。 全ファイルに伝播せず古い 352B 想定の .obj が残ると、 多数 opaque draw 時に UBO 末尾 `prevViewProj` 領域 (古い .obj から見ると未定義領域) を vertex shader が読んで `gl_Position` が NaN/巨大値 → rasterizer hang。 validation 一切無音 + 特定状態遷移時のみ再現 = デバッグ難度最高クラス。 共有ヘッダ struct size 変更は **必ず clean rebuild** (Work_Protocol §3-1a 事例②)。
