...
**(U) Timeline semaphore 受け皿** — ✅ **完了 2026-05-29 (B commit 3670ef1 受け皿 + eeba2ed FrameSync migration)**:
  - ✅ `vulkan_context::timelineSemaphore()` getter (Vulkan 1.2 core・feature 強制 enable)。
  - ✅ **FrameSync 完全 migration**: per-frame VkFence array 撤去 → 単一 timeline semaphore (`frameTimeline_` initialValue = MAX_FRAMES_IN_FLIGHT) + `nextSignalValue_` カウンタ。 acquireNextImage は `vkWaitSemaphores(value = nextSignalValue_ - MAX_FRAMES_IN_FLIGHT)`・submit は `VkTimelineSemaphoreSubmitInfo` で frameTimeline_ に signal。 binary semaphore は swapchain (acquire/present 用) に残置。
  - ✅ **副次効果**: sync_validation layer (W commit df6f5ae) が flag していた CullingPass cross-frame WRITE_AFTER_READ/WRITE hazards (cullStaging / compactCmd / countBuf) **20 件解消** (per-frame buffer 再利用が timeline 経由で explicit sync になった)。
  - 詳細パターンは Work_Protocol §5h 参照。

**(V) Async compute queue 受け皿** — ✅ **完了 (4c-B commit 477985d 受け皿 + M commit 8b4deff AsyncCompute receptacle)**:
  - ✅ `vulkan_context::asyncComputeFamily()` / `asyncComputeQueue()` / `hasDedicatedAsyncCompute()` getter (4c-B)。 P620 で family 2 dedicated 検出。
  - ✅ `include/MyEngine/renderer/async_compute.h` (M 8b4deff): `AsyncComputeContext` header-only class が単一 timeline semaphore + `nextValue()` monotonic 値払い出し + `ready()` capability check。 cross-queue submit 用の boilerplate (VkTimelineSemaphoreSubmitInfo の signal/wait 値 pair) を活性化時に集約する土台。
  - 🟡 **実 cross-queue submission は Phase 仕事** (HZB + Cull2 を asyncComputeQueue() 上で実行・graphics queue が timeline value を wait で待つ wiring)。 PART4 では受け皿だけ。 Foundations §2 と一緒に着手予定。
...
### 4c — Two-pass occlusion 本体 (§3.2-F) + AABB 遮蔽 (§0.1-C) + cull.comp 拡張 ✅ **完了 2026-05-29 (5 commits) + Tier 1 α 活性化 + 1-tap fast path**

設計時の単一 commit 想定を「machinery と activation を分けて段階的に commit」に修正して進めた。 user の「画面 OK」目視確認 + 2 bug fix 後、 Nanite/Granite 2024 baseline で完成。

| commit | 内容 |
|---|---|
| `ad97879` 4c-A | half-extent (extentDrawId.xyz) 充填 + `HiZPass::previousPyramidView()` accessor (受け皿) |
| `477985d` 4c-B | capability getters (`samplerFilterMinmax`, async compute family) + `HizParams` BDA buffer + cull.comp helpers (`aabbScreenBounds` / `mipFromScreenSize`・gate-off で dead-code) |
| `f242327` 4c-C | full machinery (HiZPass `minReductionSampler` + `ensureAllSlotsInGeneral` + `visHistory` 永続 buffer + descriptor set 0 with HZB samplers + cull.comp pass1/2 paths + main_pass Pass enum + `loadOp` 分岐) gate-off |
| `7e446a9` 4c-D | **two-pass HZB occlusion 活性化** (cull pass1 → main(FirstOpaque) → HiZPass → depth barrier → cull pass2 → main(SecondAndNonOpaque)) |
| `e41cfd7` 4c-D fix #1 | HZB descriptor set に `UPDATE_AFTER_BIND` flag 追加 (multi-dispatch 中の vkUpdateDescriptorSets が「destroyed or updated without UPDATE_AFTER_BIND」flood した bug 修正) |
| `91a6885` 4c-D fix #2 | main_pass `SecondAndNonOpaque` 経路の `toAttach` barrier をスキップ (UNDEFINED→ATTACHMENT は内容破棄なので pass1 の HDR + depth が消えて青背景黒化 + 静的 prop 消失していた bug 修正) |
| `2f7daf9` Tier 1 α | **Nanite/Granite 2024 baseline 格上げ**: pass1 が hzbPrev も sample して前フレ occluded 早期 cull (4c 時点の simpler Maister 2018 から進化、 hzbPrev binding 受け皿は 4c-A から立ってた) |
| `ccf5c03` 1-tap fast path | `samplerFilterMinmax` + reductionMode=MIN sampler 経由で 1 textureLod = min(2x2) を spec const 分岐で活用 (4-tap fallback あり) |

最終形:
- `CullingPass::ExecuteInfo` に `passIndex / hizPrevView / hizCurrView / hizSampler / twoPassEnabled` 受け皿 (twoPassEnabled で legacy fallback 経路維持)
- pass1: `prevVis && frustum && cone && !hzbOccluded(hzbPrev)` (Tier 1 α 活性後)
- pass2: `frustum && cone && !hzbOccluded(hzbCurr) && !drewByPass1` (visHistory writeback)
- visHistory = 永続 1 bit/drawId (per-thread own bit, atomic write race-free)
- HZB sample: spec const `kHzbMinReductionFastPath` で 1-tap / 4-tap 分岐
- main_pass `Pass enum`: `Single / FirstOpaque / SecondAndNonOpaque` (loadOp + skip non-opaque on FirstOpaque)

**検証** (user 「画面正常」目視確認): 全 prop / 反射 / shadow / grass / Player 正常描画。 validation エラー 0 (chest 開封時の transparent MRT warning は 4a-2 pre-existing で別軸)。

### 4d — 能力ゲート確定 + フォールバック整備 + 隣接最尖端の受け皿 + 仕上げ ✅ **大半完了 2026-05-29 (γ × 3 + M × 3 + N × 4 = 10 commits) + α/β**

設計時の「2-3 commit」を audit-driven の細分化で進めた。 4c 完了直後の user 主導 audit (「最新技術を取りこぼしている箇所がないか」) で複数 round の missed-tech 抽出 → 取り込み。

**α + β (audit 第 1 弾)**:
| commit | 内容 |
|---|---|
| `082d792` α (4b Obs B resolved) | `HiZPass::initialTransitionToGeneral` の `VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT` → `VK_PIPELINE_STAGE_2_NONE` (sync2 best practice for UNDEFINED→X) |
| `ccf5c03` β | (上記 1-tap fast path・cull.comp に分類済) |

**γ × 3 (audit 第 1 弾・dynamic rendering 完全移行)**:
| commit | 内容 |
|---|---|
| `4b9c32c` γ-1 | `PostPass` を VK 1.3 dynamic rendering 化 (VkRenderPass + VkFramebuffer 撤去・swapchain UNDEFINED→COLOR_ATTACHMENT→PRESENT_SRC_KHR 明示 barrier) |
| `da74526` γ-2 | `ShadowPass` を dynamic rendering 化 (depth-only attachment・`ShadowPipelineParams` に `VkPipelineCache` 追加で M1 も同時配線) |
| `33e1511` γ-3 | `ReflectionPass` を dynamic rendering 化 + **bonus**: startup の outNormal/outMotion validation warning 4 件が消滅 (reflection が triangle.frag を 1-attachment renderPass で作成していた legacy mismatch が解消) |

これで engine 全体 (src/renderer grep) で **VkRenderPass / VkFramebuffer の実 API 使用ゼロ** = Vulkan 1.3 dynamic-rendering native。

**M × 3 (audit 第 2 弾・最新 1.3/1.4 core 取り込み)**:
| commit | 内容 |
|---|---|
| `47c3571` M3 | `VK_KHR_dynamic_rendering_local_read` (Vulkan 1.4 core) capability 受け皿 = Phase 3 SS 効果 / tile-based fast path |
| `a62b7f0` M1 | **persistent VkPipelineCache** (Vulkan13 §3 Y closed): `<AppData>/MyEngine/MyEngine/pipeline.cache` に load/save。 全 14 vkCreate*Pipelines callsite が `ctx_->pipelineCache()` 経由。 user clean exit で 490KB 書き出し実証。 |
| `fcef5ab` M2 | `depth_layouts.h` ヘルパ撤去 → sync2 generic `VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL` / `READ_ONLY_OPTIMAL` 一括置換 (19 callsites + ヘッダ削除・コード -45 行) |

**N × 4 (audit 第 3 弾・pipeline 周辺の最新化)**:
| commit | 内容 |
|---|---|
| `7298968` N1 | `pipelineCreationCacheControl` (Vulkan 1.3 core) enable・open-world streaming で `VK_PIPELINE_CREATE_FAIL_ON_PIPELINE_COMPILE_REQUIRED_BIT` 経由の hitch 防止受け皿 |
| `c01c2e5` N4 | **`VkPhysicalDeviceVulkan14Features` chain 追加** (engine が API 1.4 で動作中なのに 1.4 features を一切 enable してない構造欠陥を修正)・`maintenance5` / `maintenance6` enable・M3 の KHR struct を canonical 1.4 form に統一 |
| `1481049` N2+N3 | `VK_EXT_graphics_pipeline_library` + `VK_KHR_pipeline_binary` capability 受け皿 (extension name walk による query のみ・activation は Phase 2A/3 で) |
...
**4d で残った仕事** (別 commit で着手可・本 PART4 のスコープ外でもよい):
...
- ~~`lastCullGpuVisible_` HUD GPU readback 配線 or 「純 GPU-driven 化」で readback 撤去~~ ✅ **完了 2026-05-29 commit f8d1e1f** (option B 採用 = HUD `Cull` 行 + CPU Frustum オラクル + 全 wire-up 撤去)
- transparent MRT mismatch fix (chest 開封時の outNormal/outMotion validation noise・4a-2 由来)
- 4b Obs C (R32G32_SFLOAT storage image format properties query)
- 4b Obs D (subgroup ID → linearIdx canonical mapping)

これら残作業は **PART4 の完了を阻まない** (open-world 動作には支障なし)。

**2026-05-29 最新化マラソン 28 commits で追加進捗**: Timeline semaphore (U) は ✅ FrameSync migration 完了 (B commit eeba2ed)・Async compute (V) は ✅ AsyncContext 受け皿確保 (M commit 8b4deff)・別軸で A1-A6 (raw vkAllocateMemory ZERO) / E + E clean (camera-relative 10 site wire-up) / F1-F5 (固定容量 5 クラス動的化) / G (bindless free-list) / N (memory_priority 実利用) / O (debug_utils GPU markers) / W + W fix (sync_validation + swapchain hazard fix) / C (transfer queue) / I (memory_budget) / D+L+K+T+Z+J+Q 受け皿 → 5 enable / U (JobSystem 受け皿) / V/R/S/H/X/Y/P (7 design memos) を完了。 詳細は START_HERE §2 / Codebase_Guide rev.16 / Foundations_Audit rev.7 / Vulkan13_Modernization rev.4 / INDEX rev.7。

次のメジャー仕事候補は **mailbox present mode + K activation (frame pacing 完成) / L 本実装 (VkPipeline 撤廃 = modern triad 完成) / S compute skinning 本実装 / M activation = 実 cross-queue submit / V-P stub の本実装 (per-Phase) / 2C LOD / Phase 2F terrain bucket / 1I PART D bloom settings 連携 / 2A 多光源 (clustered forward+)**。
...
- ~~**Obs B: 初期 layout 遷移の stage flag が legacy 形式**~~ ✅ **解決済み 2026-05-29 commit `082d792`** (4d α)
  - `VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT` → `VK_PIPELINE_STAGE_2_NONE` 置換完了。 sync2 best practice 適合。
