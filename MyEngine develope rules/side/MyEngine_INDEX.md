# MyEngine 設計知見 — 横断インデックス (rev.10)

最終更新: 2026-05-30 (rev.10: **Phase 2G-2a 完了 (skinned passthrough・旧 vertex-shader skinning 撤去・HEAD 1b57f8e)**。 triangle_skinned / shadow_skinned を passthrough 化 = 2G-1 の compute-skinned stream を BDA + gl_VertexIndex で pull (pos fp32 / normal oct16 decode)・bone math 撤去。 per-draw を SkinnedDrawData SSBO (gl_InstanceIndex・prop と同形の indirect-ready) + SkinnedDrawDataPool + skinned_draw.h の PreparedSkinnedDraw (pass_chain で1回構築し main/shadow/reflection 共有・per-pass scene re-walk なし) に。 main/shadow/reflection の drawSkinnedList CPU push ループを drawSkinnedPrepared に置換・**旧 SkinnedPushConstants / ShadowSkinnedPushConstants 全撤去 (残存参照ゼロ確認)**。 streams は model-local 保持で 3 view 共有 = skin once 達成 (3〜4x → 1x)。 §3-1a clean rebuild OK・user runtime 検証済 (skinned キャラ/影/反射/F12 burst 正常・no-op でなく出所変更だが見た目一致)。 §1 監査テーブルの skinned 行を 2G-2a 完了に更新。 残 2G-2b (skinned → CullingPass CullSet + vkCmdDrawIndexedIndirect = CPU draw ループ撤去) / 2G-3 (motion vector double-buffer + LDS bone cache + async overlap)。 / rev.9: **Phase 2G-1 完了 (batched compute skinning pre-pass)**。 skinning.comp (1 dispatch・flat global-vertex-id binary search・pos fp32x3 ping-pong + normal oct16 deinterleaved) + SkinnedVertexPool (device-local 出力) + SkinInstancePool (host-mapped 入力テーブル) + SkinningPass (descriptor-less compute・sync2 barrier) を reflection 前に dispatch。 draw は旧 vertex-shader skinning のまま = 描画 no-op (受け皿)。 HEAD 8cbf82a・user runtime 検証済 (通常起動 no-op + F12 burst クラッシュなし)。 F12 burst で grow-mid-loop device-lost を踏み pre-grow-once で修正。 §1 監査テーブルの skinned 行を「2G-1 完了」に更新。 残 2G-2 (passthrough vert + indirect 統合 + 旧経路撤去) / 2G-3 (motion vector double-buffer + LDS bone cache + async overlap)。 / rev.8: **描画経路 全監査で §1.5-B 違反 + 受け皿 S 誤設計を確定**。 §1 表の後に「2026-05-30 描画経路 全監査の結果」テーブル新設 (skinned 全経路 CPU draw ループ + grass CPU per-blade cull = §1.5-B 違反 → Phase 2G/2H 新設・受け皿 S は per-mesh 固定で複数インスタンス非対応の誤設計 → 2G で再設計・他 9 受け皿は形健全)。 §7 次推奨で S を ★★★ Phase 2G として最優先に格上げ。 監査の契機: user 指示「最新技術より既存整合・最小構成を選んでしまったかもしれないので確認」→ session 全選択を監査し Q8b (CPU skinning fallback 温存を §3 誤用で推奨) / Q9 (フル Vertex 維持を安全と化粧して推奨) の §0 違反を開示・是正。 / rev.7: **最新化マラソン 28 commits 反映 — 多数項目が ✅実装完了 / 🟢受け皿確保**。 §1 表で:
...
| 項目 | 監査結果 | 是正 |
|---|---|---|
| **§1.5-B 違反: skinned 全経路 CPU draw ループ** | opaque/transparent/shadow/reflection 全部 CPU ループ + vertex-shader skinning 3〜4 回再実行 (main_pass drawSkinnedList / shadow_pass skinned / reflection_pass drawSkinnedList) | **Phase 2G 着手中** = compute skinning packed VB + prop と同じ indirect 統合 + 旧 skinning 撤去。 **2G-1 完了 (2026-05-30 HEAD 8cbf82a・runtime 検証済)** = batched compute skinning pre-pass (skinning.comp 1 dispatch + SkinnedVertexPool deinterleaved pos fp32 ping-pong / normal oct16 + SkinInstancePool + SkinningPass) を reflection 前に配線・draw は旧経路のまま = 描画 no-op・F12 burst grow-mid-loop device-lost を pre-grow once で修正。 **2G-2a 完了 (2026-05-30 HEAD 1b57f8e・runtime 検証済 user OK)** = passthrough vert (triangle_skinned / shadow_skinned が skinned stream を BDA + gl_VertexIndex で pull・bone math 撤去) + SkinnedDrawData SSBO (gl_InstanceIndex・indirect-ready) + SkinnedDrawDataPool + skinned_draw.h (PreparedSkinnedDraw を pass_chain で1回構築し main/shadow/reflection 共有) + main/shadow/reflection を drawSkinnedPrepared 化・**旧 vertex-shader skinning と旧 SkinnedPushConstants/ShadowSkinnedPushConstants を全撤去 (残存参照ゼロ)**。 §3-1a clean rebuild OK・skinned キャラ/影/反射/F12 burst 正常。 skin once 達成 (shadow+main+reflection の 3〜4x 再 skinning を 1x に)。 残 2G-2b (skinned を CullingPass CullSet + vkCmdDrawIndexedIndirect = CPU draw ループ撤去) / 2G-3 (motion vector double-buffer + LDS bone cache + async overlap)。 |
