...
void WorldBuilder::reset(WorldData& data, bool keepPlayer) {
    ...
    destructAll(data.potionItems);
    destructAll(data.spiritItems);
    destructAll(data.chests);
    destructAll(data.graves);
    data.obstacles.clear();
    // CWater を持つ entity を destruct (= scene_builder のループで使われてるもの)
    {
        std::vector<flecs::entity> waterEnts;
        data.world.each([&](flecs::entity e, const CWater&) { waterEnts.push_back(e); });
        for (flecs::entity e : waterEnts) {
            if (e.is_alive()) e.destruct();
        }
    }
    {
    ...
    if (!data.terrains.empty() || !data.waters.empty()) {
