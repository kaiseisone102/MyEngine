// =============================================================================
// choice_overlay_layer.cpp — 真のオーバーレイ版
// =============================================================================
#define NOMINMAX
...
ChoiceOverlayLayer::ChoiceOverlayLayer(render::SceneRenderer& renderer, std::string prompt,
...
// 透明度: ダイアログ自体は不透明 (= 中身が透けて読みづらくならない)
//         バックドロップだけ半透明黒で「下の画面がうっすら見える」 効果を出す。
constexpr float kDialogBgAlpha = 1.00f;
constexpr float kBackdropAlpha = 140.f / 255.f;  // 全画面被せの半透明黒
...
void ChoiceOverlayLayer::drawImGui() {
    ...
    // ─── 1. 半透明黒のバックドロップ (全 Layer の上、 ダイアログの下) ──
    ...
    ImGui::Begin("##ChoiceOverlayBackdrop", nullptr,
    ...
    // ─── 2. ダイアログ本体 ──────────────────────────────────
    // ダイアログを確実に最前面に出すため SetNextWindowFocus を呼ぶ。
    // また WindowBg の不透明矩形が他 Window より前に描かれるよう、
    // ダイアログには NoBringToFrontOnFocus を「付けない」 ことが必須。
    // ダイアログの背景は完全不透明 (kDialogBgAlpha=1.0) で下の画面が透けない。
    ...
    ImGui::Begin("##ChoiceDialog", nullptr,
                 ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
